...
struct DoubleEndedPriorityQueue{
    ...
    DoubleEndedPriorityQueue() : data(2){}
    DoubleEndedPriorityQueue(vector<T>& array) {
        copy(array.begin(), array.end(), data.begin()+2);
        int sz = data.size()+2;
    ...
    T get_max(){
        assert(this->size() != 0);
        return this->size() > 1 ? data[1] : data[0];
    }
    ...
    int _swap(int k){
        if ((k|1) < data.size() && data[k&(-2)] < data[k|1]) {
            swap(data[k&(-2)], data[k|1]);
            k ^= 1;
        }
        return k;
    }
...
int main(){
    ...
    DoubleEndedPriorityQueue pq(init);



    cout << pq.get_max() << endl;
    cout << pq.get_min() << endl;
    pq.pop_max();
    cout << pq.get_max() << endl;
    cout << pq.get_min() << endl;
    pq.pop_min();
