...
struct PersistentSegmentTree{
    ...
    PersistentSegmentTree(vector<T> _array,function<T(T,T)> _op, function<T()> _e){
        ...
        for (int i = 0;i<_size;i++){
            tmp = new Node{nullptr,nullptr,nullptr,_array[i]};
            q.push(tmp);
        }
        while (q.size() >= 2){
            left = q.front();
            ...
            right = q.front();
            q.pop();
            tmp = new Node{nullptr,nullptr,nullptr,_op(left->value, right->value)};
            right->parent = tmp;
            left->parent = tmp;
            q.push(tmp);
        }
